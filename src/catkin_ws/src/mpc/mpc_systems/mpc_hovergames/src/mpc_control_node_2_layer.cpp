...
int main(int argc, char *argv[]) {
  ...
  if (node_name == "tracker") {
    ...
    // ptr_reconfigure_callback =
    // std::unique_ptr<ReconfigureCallback<solver_config0>>(new
    // ReconfigureCallback<solver_config0>(ptr_solver.get(), nh,
    // &loadMPCWeights0));
    ReconfigureCallback<solver_config0> test(ptr_solver.get(), nh,
                                             &loadMPCWeights0);
    loop_rate.sleep();
  } else if (node_name == "planner") {
    ptr_solver = returnSolverPointer(1);
    // ptr_reconfigure_callback =
    // std::unique_ptr<ReconfigureCallback<solver_config1>>(new
    // ReconfigureCallback<solver_config1>(ptr_solver.get(), nh,
    // &loadMPCWeights1));
    ReconfigureCallback<solver_config1> test(ptr_solver.get(), nh,
                                             &loadMPCWeights1);
    loop_rate.sleep();
  } else {
...
#ifdef PX4
  ptr_interface =
      std::unique_ptr<HovergamesPX4Interface>(new HovergamesPX4Interface(
...
#endif
#ifdef SIMPLESIM
  ptr_interface = std::unique_ptr<HovergamesSimpleSimInterface>(
      new HovergamesSimpleSimInterface(
          nh, ptr_controller.get(), &configuration_mpc, ptr_solver.get(),
          ptr_module_handler.get(), ptr_robot_region.get(),
          ptr_data_saver_json.get(), &mutex));
#endif
