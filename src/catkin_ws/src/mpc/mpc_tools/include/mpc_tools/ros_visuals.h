...
enum class Colormap { VIRIDIS = 0, INFERNO, BRUNO };

class ROSMarkerPublisher {
 public:
  ROSMarkerPublisher(ros::NodeHandle &nh, const char *topic_name,
                     const std::string &frame_id, int max_size);
  ~ROSMarkerPublisher();
 ...
 public:
  ...
  ROSMultiplePointMarker &getNewMultiplePointMarker(
      const std::string &marker_type);  // const std::string &marker_type);
  ROSTextMarker &getNewTextMarker();    // const std::string &marker_type);
  ROSModelMarker &getNewModelMarker(
      const std::string &model_path);  // const std::string &marker_type);
...
class ROSMarker {
 ...
 protected:
  ...
  void getColorFromRange(double ratio, double &red, double &green,
                         double &blue);
  void getColorFromRangeInt(int select, double &red, double &green,
                            double &blue,
                            const Colormap &colormap = Colormap::VIRIDIS);

 public:
  void setColor(double r, double g, double b, double alpha = 1.0);
  // void setColor(double ratio); // Color rainbow wise

  void setColorInt(int select, double alpha = 1.0,
                   const Colormap &&colormap =
                       Colormap::VIRIDIS);  // Viridis (select our of range)
  void setColorInt(int select, int range, double alpha = 1.0,
                   const Colormap &&colormap =
                       Colormap::VIRIDIS);  // Viridis (select our of range)
...
class ROSLine : public ROSMarker {
 public:
  ...
  void addBrokenLine(const Eigen::Vector3d &p1, const Eigen::Vector3d &p2,
                     double dist);
  void addBrokenLine(const geometry_msgs::Point &p1,
                     const geometry_msgs::Point &p2, double dist);

  void addCircularLine(const Eigen::Vector2d &center, double radius, double z,
                       double n_points);
};

class ROSPointMarker : public ROSMarker {
 public:
  ROSPointMarker(ROSMarkerPublisher *ros_publisher, const std::string &frame_id,
...
class ROSMultiplePointMarker : public ROSMarker {
 public:
  ROSMultiplePointMarker(ROSMarkerPublisher *ros_publisher,
                         const std::string &frame_id, const std::string &type);
...
class ROSModelMarker : public ROSMarker {
 public:
  ROSModelMarker(ROSMarkerPublisher *ros_publisher, const std::string &frame_id,
